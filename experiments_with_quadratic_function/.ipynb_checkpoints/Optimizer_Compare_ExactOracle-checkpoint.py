...
def LS_noisygrad(x, iter_count):
    ...
    #vec = np.dot(P, x) - b + np.random.normal(0, 0.1, (n))
...
plt.plot(ys_mgd, 'g', lw=1, label='GD w/ Momentum')
plt.plot(ys_nag, 'r', lw=1, label='NAG')
plt.plot(ys_nag_ar, 'k', lw=1, label='NAG w/ Adaptive Restart')
plt.plot(ys_nag_sr, 'm', lw=1, label='NAG w/ Scheduled Restart')
...
plt.ylabel('|f(x) - f(x*)|')
#plt.show()
plt.savefig('Quadratic_Exact_Oracle.pdf')


# Save data to txt for visualization
with open('GD_Quadratic_Exact_Oracle.txt', 'w') as filehandle:
    filehandle.writelines("%20.14f\n" % item1 for item1 in ys_gd)
with open('MGD_Quadratic_Exact_Oracle.txt', 'w') as filehandle:
    filehandle.writelines("%20.14f\n" % item1 for item1 in ys_mgd)
with open('NAG_Quadratic_Exact_Oracle.txt', 'w') as filehandle:
    filehandle.writelines("%20.14f\n" % item1 for item1 in ys_nag)
with open('NAGAR_Quadratic_Exact_Oracle.txt', 'w') as filehandle:
    filehandle.writelines("%20.14f\n" % item1 for item1 in ys_nag_ar)
with open('NAGSR_Quadratic_Exact_Oracle.txt', 'w') as filehandle:
