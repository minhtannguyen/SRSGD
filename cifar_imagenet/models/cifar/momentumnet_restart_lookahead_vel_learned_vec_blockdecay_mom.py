...
__all__ = ['momentumnet_restart_lookahead_vel_learned_vec_blockdecay_mom']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=2.0, momentum=Parameter(torch.full((1,1,1,1),0.5), requires_grad=True),blockid=0):
    ...
    def forward(self, invec):
        ...
        x = x + self.blockid/(self.blockid + 3.0) * self.momentum * prex
        ...
        if self.downsample is not None:
            ...
            outprex = residualprex - self.step_size * out
        else:   
            outprex = self.blockid/(self.blockid + 3.0) * self.momentum * residualprex - self.step_size * out
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, step_size=step_size, momentum=torch.tensor(0.0), blockid=0))
        self.inplanes = planes * block.expansion
        momentum_param = Parameter(torch.full((1,self.inplanes,1,1),momentum), requires_grad=True)
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, step_size=step_size, momentum=momentum_param, blockid=i))
...
def momentumnet_restart_lookahead_vel_learned_vec_blockdecay_mom(**kwargs):
    """
