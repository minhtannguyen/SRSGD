...
__all__ = ['momentumnet_restart_lookahead_vel_learned_scalar_uniblock_mom']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=2.0, momentum=Parameter(torch.tensor(0.5), requires_grad=True)):
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, step_size=step_size, momentum=momentum_param))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, step_size=step_size, momentum=momentum_param))
...
def momentumnet_restart_lookahead_vel_learned_scalar_uniblock_mom(**kwargs):
    """
