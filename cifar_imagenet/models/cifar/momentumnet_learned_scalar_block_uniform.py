...
__all__ = ['momentumnet_learned_scalar_block_uniform']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=Parameter(torch.tensor(2.0), requires_grad=True), momentum=Parameter(torch.tensor(0.5), requires_grad=True)):
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        downsample = None
        step_size_param = Parameter(torch.tensor(step_size), requires_grad=True)
...
def momentumnet_learned_scalar_block_uniform(**kwargs):
    """
