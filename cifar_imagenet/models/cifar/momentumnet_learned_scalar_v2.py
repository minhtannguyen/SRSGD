...
__all__ = ['momentumnet_learned_scalar_v2']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, downsampley=None, step_size=2.0, momentum=0.5):
    ...
    def forward(self, invec):
        ...
        if self.downsample is not None:
            residualx = self.downsample(x)
        if self.downsampley is not None:
            residualy = self.downsampley(y)
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsampley = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, downsampley, step_size=step_size, momentum=momentum))
...
def momentumnet_learned_scalar_v2(**kwargs):
    """
