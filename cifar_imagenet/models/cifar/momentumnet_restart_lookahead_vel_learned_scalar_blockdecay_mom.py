...
__all__ = ['momentumnet_restart_lookahead_vel_learned_scalar_blockdecay_mom']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=2.0, momentum=Parameter(torch.tensor(0.5), requires_grad=True), blockid=0):
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        downsample = None
        momentum_param = Parameter(torch.tensor(momentum), requires_grad=True)
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, step_size=step_size, momentum=momentum_param, blockid=0))
...
def momentumnet_restart_lookahead_vel_learned_scalar_blockdecay_mom(**kwargs):
    """
