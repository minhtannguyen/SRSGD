...
__all__ = ['momentumnet_heavyball_vel_scheme3_learned_scalar_v2']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, downsampleprex=None, step_size=2.0, momentum=0.5):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.downsampleprex = downsampleprex
        self.stride = stride
        # for momentum net
        self.step_size = Parameter(torch.tensor(step_size), requires_grad=True)
        self.momentum = Parameter(torch.tensor(momentum), requires_grad=True)
    ...
    def forward(self, invec):
        ...
        if self.downsampleprex is not None:
            residualprex = self.downsampleprex(residualprex)
            
        outprex = self.momentum * residualprex - out
        outx = residualx + self.step_size * outprex
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsampleprex = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, downsampleprex, step_size=step_size, momentum=momentum))
...
def momentumnet_heavyball_vel_scheme3_learned_scalar_v2(**kwargs):
    """
