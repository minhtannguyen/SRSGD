...
__all__ = ['momentumnet_restart_lookahead_vel_learned_vec']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=2.0, momentum=0.5):
        ...
        self.stride = stride
        # for momentum net
        self.step_size = Parameter(torch.full((1,planes * self.expansion,1,1),step_size), requires_grad=True)
        self.momentum = Parameter(torch.full((1,planes * self.expansion,1,1),momentum), requires_grad=True)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=2.0, momentum=0.5):
        ...
        # for momentum net
        self.step_size = Parameter(torch.full((1,planes * self.expansion,1,1),step_size), requires_grad=True)
        self.momentum = Parameter(torch.full((1,inplanes,1,1),momentum), requires_grad=True)
...
def momentumnet_restart_lookahead_vel_learned_vec(**kwargs):
    """
