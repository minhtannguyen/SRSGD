...
class CifarResNeXt(nn.Module):
  ...
  def forward_once(self, x, cycles, coeff, mse_parameter=0.1, kl_parameter=0.1, tv_parameter=0.1):
    x = self.conv_1_3x3(x)
  ...
  def forward(self, x, cycles, coeff, mse_parameter=0.1, kl_parameter=0.1, tv_parameter=0.1):
    for i_cycle in range(cycles):
        # feedforward
        output = self.forward_once(x_cur, null_space=None, unit_space=None)
