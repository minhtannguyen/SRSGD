...
__all__ = ['momentumnet_learned_vec_block_uniform']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, step_size=Parameter(torch.full((1,1,1,1),2.0), requires_grad=True), momentum=Parameter(torch.full((1,1,1,1),0.5), requires_grad=True)):
...
class MomentumNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, step_size=2.0, momentum=0.5):
        downsample = None
        step_size_param = Parameter(torch.full((1,planes * block.expansion,1,1),step_size), requires_grad=True)
        momentum_param = Parameter(torch.full((1,planes * block.expansion,1,1),momentum), requires_grad=True)
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, step_size=step_size_param, momentum=momentum_param))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, step_size=step_size_param, momentum=momentum_param))
...
def momentumnet_learned_vec_block_uniform(**kwargs):
    """
